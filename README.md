...
- [cite_start]**🚨 Cảnh báo cháy (Ưu tiên tuyệt đối):** Khi nhiệt độ vượt quá 50°C, hệ thống lập tức cưỡng chế tắt quạt (ngăn cung cấp oxy) và nháy đèn LED cảnh báo [cite: 46-48]. [cite_start]Hệ thống có cơ chế Hysteresis, chỉ tắt báo động khi nhiệt độ giảm xuống dưới 45°C[cite: 31, 48].
- [cite_start]**🌡️ Quạt thông minh (Smart Fan):** Tự động điều chỉnh 3 tốc độ gió theo nhiệt độ (28°C, 30°C, 32°C) nhưng **chỉ hoạt động khi có người** để tiết kiệm năng lượng [cite: 40-42].
- [cite_start]**💡 Đèn thông minh (Smart Light):** Tự động bật sáng khi phát hiện trời tối VÀ có người di chuyển [cite: 44-45].
- [cite_start]**🔄 Tự động phục hồi (Auto-Revert):** Khi người dùng can thiệp chỉnh tay (Manual), hệ thống sẽ đếm ngược 10 giây (nếu không có thao tác mới) để tự động quay về chế độ Auto [cite: 50-51].
...
1.  [cite_start]**Tầng Vật lý (Hardware):** Cảm biến (DHT11, PIR, LDR) + Thiết bị chấp hành (Quạt, Đèn) $\rightarrow$ Vi điều khiển trung tâm **Arduino Uno**[cite: 22].
2.  [cite_start]**Tầng Giao tiếp Nội bộ:** Arduino Uno đóng gói dữ liệu (7 Bytes) và gửi sang **ESP32** qua giao thức **I2C** [cite: 67-69].
3.  [cite_start]**Tầng Mạng (Network):** ESP32 đóng gói JSON và truyền qua WiFi đến Broker **HiveMQ Cloud** (MQTT over SSL - Port 8883) [cite: 2, 16-18].
...
2.  Mở `mqtt_esp.ino`, cập nhật thông tin mạng của bạn:
    ...
    ```
3.  Nạp code vào **ESP32**. Đảm bảo nối dây I2C giữa 2 board (SDA_UNO $\leftrightarrow$ D21_ESP32, SCL_UNO $\leftrightarrow$ D22_ESP32) và nối chung mass (GND)[cite: 3].
...
2.  Kiểm tra và cập nhật thông tin HiveMQ (Server, Port, Username, Password) tại 3 file:
    - [cite_start]`mqtt_esp.ino` [cite: 2]
    - `worker.php`
    - `index.php`
